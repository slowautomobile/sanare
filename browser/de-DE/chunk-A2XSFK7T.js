import { MatButton, MatButtonModule } from "./chunk-ETCGSIPQ.js";
import { BasePortalOutlet, CdkPortalOutlet, ComponentPortal, DefaultValueAccessor, FormBuilder, FormControlName, FormGroupDirective, FormsModule, HttpClient, NgControl, NgControlStatus, NgControlStatusGroup, NgForm, Overlay, OverlayConfig, OverlayModule, PortalModule, ReactiveFormsModule, TemplatePortal, Title, Validators, coerceBooleanProperty, getSupportedInputTypes, ɵNgNoValidate } from "./chunk-LFO7H5VY.js";
import { BreakpointObserver, Directionality, LiveAnnouncer, MatCommonModule, ObserversModule, Platform, _CdkPrivateStyleLoader, _IdGenerator, _bindEventWithOptions, coerceElement, coerceNumberProperty } from "./chunk-VZWK6WXM.js";
import { ANIMATION_MODULE_TYPE, APP_BOOTSTRAP_LISTENER, APP_INITIALIZER, ApplicationRef, Attribute, BehaviorSubject, ChangeDetectionStrategy, ChangeDetectorRef, Compiler, Component, ConnectableObservable, Console, ContentChild, ContentChildren, DOCUMENT, DestroyRef, Directive, EMPTY, ENVIRONMENT_INITIALIZER, ElementRef, EmptyError, EnvironmentInjector, EventEmitter, HashLocationStrategy, HostAttributeToken, HostBinding, HostListener, InjectFlags, Injectable, InjectionToken, Injector, Input, LOCATION_INITIALIZED, Location, LocationStrategy, NgModule, NgModuleFactory$1, NgTemplateOutlet, NgZone, Observable, Optional, Output, PathLocationStrategy, PendingTasksInternal, Renderer2, RendererFactory2, RuntimeError, SkipSelf, Subject, Subscription, TemplateRef, Version, ViewChild, ViewContainerRef, ViewEncapsulation, ViewportScroller, __spreadProps, __spreadValues, afterNextRender, afterRender, auditTime, booleanAttribute, catchError, combineLatest, computed, concat, concatMap, contentChild, createEnvironmentInjector, defaultIfEmpty, defer, effect, filter, finalize, first, from, inject, input, isInjectable, isNgModule, isObservable, isPromise, isSignal, isStandalone, last, makeEnvironmentProviders, map, merge, mergeAll, mergeMap, of, pairwise, pipe, refCount, reflectComponentType, runInInjectionContext, scan, setClassMetadata, shareReplay, signal, startWith, switchMap, take, takeLast, takeUntil, tap, throwError, ɵsetClassDebugInfo, ɵɵInheritDefinitionFeature, ɵɵNgOnChangesFeature, ɵɵProvidersFeature, ɵɵadvance, ɵɵattribute, ɵɵclassProp, ɵɵconditional, ɵɵcontentQuery, ɵɵcontentQuerySignal, ɵɵdefineComponent, ɵɵdefineDirective, ɵɵdefineInjectable, ɵɵdefineInjector, ɵɵdefineNgModule, ɵɵdirectiveInject, ɵɵelement, ɵɵelementEnd, ɵɵelementStart, ɵɵgetCurrentView, ɵɵgetInheritedFactory, ɵɵhostProperty, ɵɵi18n, ɵɵinject, ɵɵinjectAttribute, ɵɵinvalidFactory, ɵɵlistener, ɵɵloadQuery, ɵɵnextContext, ɵɵprojection, ɵɵprojectionDef, ɵɵproperty, ɵɵqueryAdvance, ɵɵqueryRefresh, ɵɵreference, ɵɵresetView, ɵɵrestoreView, ɵɵsanitizeUrlOrResourceUrl, ɵɵtemplate, ɵɵtemplateRefExtractor, ɵɵtext, ɵɵtextInterpolate, ɵɵtextInterpolate1, ɵɵviewQuery } from "./chunk-TTKC63AX.js";
...
var AuthService = class _AuthService {
  constructor() {
    ...
    this.apiURLUsers = environment.apiURL + "users";
  }
...
var AuthGuard = class _AuthGuard {
  constructor() {
    this.router = inject(Router);
    this.localStorageToken = inject(LocalstorageService);
  }
  canActivate() {
  ...
  _tokenExpired(expiration) {
    return Math.floor((/* @__PURE__ */new Date()).getTime() / 1e3) >= expiration;
  }
  static {
...
var UsersService = class _UsersService {
  constructor() {
    ...
    this.profileThumbnail = signal("");
    this.userFavorites = signal({
      favExercises: [],
      favPrograms: []
    });
    this.favoritePrograms = signal([]);
    this.favoriteExercises = signal([]);
  }
  getUsers() {
  ...
  getFavorites(userId) {
    return this.http.get(`${this.apiURLUsers}/favorites/${userId}`).pipe(map(favoritesData => {
      console.log("Get favorites data service: ", favoritesData);
      this.userName.set(favoritesData.firstName + " " + favoritesData.lastName);
      this.profileThumbnail.set(favoritesData.profilePictureThumb);
      this.favoritePrograms.set(favoritesData.favorites.favPrograms);
      this.favoriteExercises.set(favoritesData.favorites.favExercises);
      const normalizedFavorites = __spreadProps(__spreadValues({}, favoritesData.favorites), {
        favPrograms: favoritesData.favorites.favPrograms.map(program => program.id),
        favExercises: favoritesData.favorites.favExercises.map(exercise => exercise.id)
      });
      return normalizedFavorites;
    }));
  }
  updateFavorites(userId, favorites) {
    return this.http.patch(`${this.apiURLUsers}/favorites/${userId}`, favorites).pipe(map(favoritesData => {
      console.log("Favorites data after update: ", favoritesData);
      this.favoritePrograms.set(favoritesData.favorites.favPrograms);
      this.favoriteExercises.set(favoritesData.favorites.favExercises);
  ...
  getUserLanguage(userId) {
    return this.http.get(`${this.apiURLUsers}/user-language/${userId}`);
  }
  /*   getProfileThumbnail(userId: string): Observable<string> {
    return this.http.get<string>(
      `${this.apiURLUsers}/profilePicThumb/${userId}`,
    );
  } */
  updateUserLanguage(userId, userLanguageCode) {
    return this.http.patch(`${this.apiURLUsers}/user-language/${userId}`, userLanguageCode);
  }
  loginAsGuest() {
    return this.http.get(`${this.apiURLUsers}/guest/login`);
  }
  static {
...
function LoginComponent_Conditional_24_Template(rf, ctx) {
  if (rf & 1) {
    ...
    ɵɵtext(1, "Email and/or password are incorrect");
    ɵɵelementEnd();
  }
}
var LoginComponent = class _LoginComponent {
  ...
  onSubmit() {
    ...
    const subscription = this.auth.login(this.loginForm["email"].value, this.loginForm["password"].value).subscribe({
      next: user => {
        this.authError = false;
        ...
        if (token) {
          ...
          if (tokenDecode.userLanguage) {
            this.localstorageService.setToken("userLanguage", tokenDecode.userLanguage);
          }
          if (isAdminPath) {
            this.router.navigate(["/"]);
          } else {
            console.log("navigate to lang build");
            location.replace(`${window.location.origin}/${tokenDecode.userLanguage}/`);
          }
  ...
  onSubmitGuest() {
    this.usersService.loginAsGuest().subscribe({
      next: data => {
        ...
        this.localstorageService.setToken("jwtToken", token);
        this.localstorageService.setToken("userLanguage", "en-US");
        this.router.navigate(["/"]);
      },
      error: err => {
        this._snackBar.open("Zu viele Anmeldeversuche. Versuchen Sie es in 15 Minuten erneut.", "", {
          duration: 2e3,
  ...
  static {
    this.ɵcmp = /* @__PURE__ */ɵɵdefineComponent({
      ...
      consts: () => {
        let i18n_0;
        if (false) {
          const MSG_EXTERNAL_Login_emailPlaceholder$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_0 = goog.getMsg("Enter username/email");
          i18n_0 = MSG_EXTERNAL_Login_emailPlaceholder$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_0;
        } else {
          i18n_0 = "Benutzername/E-Mail eingeben";
        }
        let i18n_1;
        if (false) {
          const MSG_EXTERNAL_Login_passwordPlaceholder$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_1 = goog.getMsg("Enter password");
          i18n_1 = MSG_EXTERNAL_Login_passwordPlaceholder$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_1;
        } else {
          i18n_1 = "Passwort eingeben";
        }
        let i18n_2;
        if (false) {
          const MSG_EXTERNAL_Login_signIn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_2 = goog.getMsg("Sign in");
          i18n_2 = MSG_EXTERNAL_Login_signIn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_2;
        } else {
          i18n_2 = "Anmelden";
        }
        let i18n_3;
        if (false) {
          const MSG_EXTERNAL_Login_continueToSanare$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_3 = goog.getMsg("to continue to Sanare");
          i18n_3 = MSG_EXTERNAL_Login_continueToSanare$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_3;
        } else {
          i18n_3 = "weiter nach Sanare";
        }
        let i18n_4;
        if (false) {
          const MSG_EXTERNAL_ClientsList_tableEmailColumn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_4 = goog.getMsg("Email");
          i18n_4 = MSG_EXTERNAL_ClientsList_tableEmailColumn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_4;
        } else {
          i18n_4 = "E-Mail";
        }
        let i18n_5;
        if (false) {
          const MSG_EXTERNAL_Login_password$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_5 = goog.getMsg("Password");
          i18n_5 = MSG_EXTERNAL_Login_password$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_5;
        } else {
          i18n_5 = "Passwort";
        }
        let i18n_6;
        if (false) {
          const MSG_EXTERNAL_Login_createBtn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_6 = goog.getMsg(" Create account ");
          i18n_6 = MSG_EXTERNAL_Login_createBtn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_6;
        } else {
          i18n_6 = " Konto erstellen";
        }
        let i18n_7;
        if (false) {
          const MSG_EXTERNAL_Login_submitBtn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_7 = goog.getMsg(" Submit ");
          i18n_7 = MSG_EXTERNAL_Login_submitBtn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_7;
        } else {
          i18n_7 = " Absenden ";
        }
        let i18n_8;
        if (false) {
          const MSG_EXTERNAL_Login_tryAppBtn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_8 = goog.getMsg(" Try App as Guest ");
          i18n_8 = MSG_EXTERNAL_Login_tryAppBtn$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_8;
        } else {
          i18n_8 = "Als Gast fortfahren";
        }
        let i18n_9;
        if (false) {
          const MSG_EXTERNAL_Login_enterEmailError$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_9 = goog.getMsg("Please enter a valid email address");
          i18n_9 = MSG_EXTERNAL_Login_enterEmailError$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_9;
        } else {
          i18n_9 = "Bitte geben Sie eine g\xFCltige E-Mail-Adresse ein";
        }
        let i18n_10;
        if (false) {
          const MSG_EXTERNAL_Login_emailRequiredError$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_10 = goog.getMsg("Email is required");
          i18n_10 = MSG_EXTERNAL_Login_emailRequiredError$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_10;
        } else {
          i18n_10 = "E-Mail ist erforderlich";
        }
        let i18n_11;
        if (false) {
          const MSG_EXTERNAL_Login_passwordRequiredError$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_11 = goog.getMsg("Password is required");
          i18n_11 = MSG_EXTERNAL_Login_passwordRequiredError$$LIBS_USERS_SRC_LIB_PAGES_LOGIN_LOGIN_COMPONENT_TS_11;
        } else {
          i18n_11 = "Passwort ist erforderlich";
        }
        return [i18n_2, i18n_3, i18n_4, i18n_5, i18n_6, i18n_7, i18n_8, i18n_9, i18n_10, i18n_11, [1, "login-register"], [1, "login-panel"], [1, "flex-item1", "flex-item"], [1, "logo"], ["src", "assets/images/leaf-img.svg", "alt", "SanareApp logo"], [1, "sign-in"], [1, "mat-display-medium"], [1, "flex-item2", "flex-item"], [1, "login-form"], [3, "formGroup"], ["appearance", "outline", 1, "form-field-full-width"], ["type", "email", "matInput", "", "formControlName", "email", "placeholder", i18n_0], ["type", "password", "matInput", "", "formControlName", "password", "placeholder", i18n_1], ["18n", "@@Login.emailOrPasswordIncorrect", 1, "error"], [1, "buttons", "flex-item3", "flex-item"], [1, "create-submit-flex"], ["mat-stroked-button", "", "disabled", ""], ["mat-flat-button", "", 3, "click", "disabled"], ["mat-flat-button", "", 1, "guest-login-button", 3, "click", "disabled"]];
      },
      template: function LoginComponent_Template(rf, ctx) {
        if (rf & 1) {
          ...
          ɵɵelement(4, "img", 14);
          ɵɵelementEnd();
          ɵɵelementStart(5, "div", 15)(6, "h1", 16);
          ɵɵi18n(7, 0);
          ɵɵelementEnd();
          ɵɵelementStart(8, "span");
          ɵɵi18n(9, 1);
          ɵɵelementEnd()()();
          ɵɵelementStart(10, "div", 17)(11, "div", 18)(12, "form", 19)(13, "mat-form-field", 20)(14, "mat-label");
          ɵɵi18n(15, 2);
          ɵɵelementEnd();
          ...
          ɵɵelementEnd();
          ɵɵelementStart(19, "mat-form-field", 20)(20, "mat-label");
          ɵɵi18n(21, 3);
          ɵɵelementEnd();
          ɵɵelement(22, "input", 22);
          ɵɵtemplate(23, LoginComponent_Conditional_23_Template, 2, 0, "mat-error")(24, LoginComponent_Conditional_24_Template, 2, 0, "mat-hint", 23);
          ɵɵelementEnd()()()();
          ɵɵelementStart(25, "div", 24)(26, "div", 25)(27, "button", 26);
          ɵɵi18n(28, 4);
          ɵɵelementEnd();
        ...
        if (rf & 2) {
          ɵɵadvance(12);
          ɵɵproperty("formGroup", ctx.loginFormGroup);
          ɵɵadvance(5);
          ɵɵconditional(ctx.loginForm["email"].hasError("email") && !ctx.loginForm["email"].hasError("required") ? 17 : -1);
          ɵɵadvance();
          ɵɵconditional(ctx.loginForm["email"].hasError("required") ? 18 : -1);
          ɵɵadvance(5);
          ɵɵconditional(ctx.loginForm["password"].hasError("required") ? 23 : -1);
          ɵɵadvance();
          ɵɵconditional(ctx.authError ? 24 : -1);
          ɵɵadvance(5);
          ɵɵclassProp("spinner", ctx.isSubmitted);
          ɵɵproperty("disabled", ctx.isSubmitted);
          ɵɵadvance(2);
          ɵɵclassProp("spinner", ctx.isGuestSubmitted);
          ɵɵproperty("disabled", ctx.isGuestSubmitted);
        }
      },
      dependencies: [FormsModule, ɵNgNoValidate, DefaultValueAccessor, NgControlStatus, NgControlStatusGroup, ReactiveFormsModule, FormGroupDirective, FormControlName, MatInputModule, MatInput, MatFormField, MatLabel, MatHint, MatError, MatFormFieldModule, MatButtonModule, MatButton],
      styles: ['.login-register[_ngcontent-%COMP%]{display:flex;flex-direction:column;justify-content:center;align-items:center;min-height:100vh}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]{width:100%;height:100vh;padding:36px;display:flex;flex-direction:column;gap:32px;justify-content:flex-start}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .flex-item1[_ngcontent-%COMP%]   .logo[_ngcontent-%COMP%]{width:48px;margin-bottom:16px}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .flex-item1[_ngcontent-%COMP%]   .logo[_ngcontent-%COMP%]   img[_ngcontent-%COMP%]{object-fit:contain;width:100%}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .flex-item2[_ngcontent-%COMP%]{display:flex;flex-direction:column;justify-content:center}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .flex-item2[_ngcontent-%COMP%]   .login-form[_ngcontent-%COMP%]   .form-field-full-width[_ngcontent-%COMP%], .login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .flex-item3[_ngcontent-%COMP%]{width:100%}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .buttons[_ngcontent-%COMP%]{align-self:flex-end;display:flex;flex-wrap:wrap;gap:24px;justify-content:flex-end}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .create-submit-flex[_ngcontent-%COMP%]{display:flex;gap:24px;width:100%}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .create-submit-flex[_ngcontent-%COMP%]   button[_ngcontent-%COMP%]{flex:1}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .guest-login-button[_ngcontent-%COMP%]{width:100%}@keyframes _ngcontent-%COMP%_spinner{to{transform:rotate(360deg)}}.spinner[_ngcontent-%COMP%]:before{content:"";box-sizing:border-box;position:absolute;top:50%;left:50%;width:20px;height:20px;margin-top:-10px;margin-left:-10px;border-radius:50%;border:2px solid #ffffff;border-top-color:#000;animation:_ngcontent-%COMP%_spinner .8s linear infinite}@media (min-width: 600px){.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]{height:auto;justify-content:space-between;border-radius:28px;width:480px}}@media (min-width: 960px){.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]{min-width:840px;flex-direction:row;height:402px;flex-wrap:wrap}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .flex-item[_ngcontent-%COMP%]{flex:0 0 45%}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .flex-item2[_ngcontent-%COMP%]{align-self:flex-end}.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]   .flex-item3[_ngcontent-%COMP%]{margin-left:auto}}@media (min-width: 1240px){.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]{width:70vw}}@media (min-width: 1600px){.login-register[_ngcontent-%COMP%]   .login-panel[_ngcontent-%COMP%]{width:1040px}}\n/*# sourceMappingURL=login.component-QGUINP2H.css.map */']
    });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && ɵsetClassDebugInfo(LoginComponent, {
    ...
    filePath: "libs/users/src/lib/pages/login/login.component.ts",
    lineNumber: 37
  });
})();

// libs/users/src/lib/users.routes.ts
var usersRoutes = [{
  path: "",
  title: "Login \u2981 Sanare",
  component: LoginComponent
}];
export { environment, ActivatedRoute, RouterOutlet, Router, RouterLink, RouterLinkActive, provideRouter, withEnabledBlockingInitialNavigation, withComponentInputBinding, RouterModule, LocalstorageService, AuthService, AuthGuard, CdkTextareaAutosize, MAT_INPUT_VALUE_ACCESSOR, SharedResizeObserver, MatLabel, MatError, MatHint, MatPrefix, MatSuffix, MatFormFieldControl, MAT_FORM_FIELD, MatFormField, ErrorStateMatcher, _ErrorStateTracker, MatFormFieldModule, MatInput, MatInputModule, UsersService, MatSnackBar, LoginComponent, usersRoutes };
/**i18n:e33dda9241aa0d3f81605574177eca303a560132442e585b3107f4f1b4a47a86*/
//# sourceMappingURL=chunk-A2XSFK7T.js.map